...
class PruebaNapakalaki
  ...
  def self.monstersMoreLevelThan10()
   @@monsters.select { |m| m.combatLevel >= 10 }
  end
  
  def self.monstersLoseLevels()
    @@monsters.select { |m| 
      m.bc.levels > 0 and not m.bc.nVisibleTreasures and not 
      m.bc.nHiddenTreasures
      }
  end
  ...
  def self.monsterLoseTreasures(tipo)
    @@monsters.select { |m|
      m.bc.specificVisibleTreasures.member?(tipo) or
      m.bc.specificHiddenTreasures.member?(tipo)
    }
  end
  ...
  def createMonsters
    ...
    @@monsters << Monster.new('El rey rosado',13,badConsequence, price)
#    
    # Flecher
    p8 = Prize.new(1, 1)
...
imprimir(self.monstersMoreLevelThan10(), "Monstruos con nivel 10")
